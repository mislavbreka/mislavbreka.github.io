...
const setCourseNum = (course, num) => {
  ...
  course.childNodes[3].childNodes[3].setAttribute(
    "oninput",
    `addCourse(${num})`
  );
  ...
  course.childNodes[5].childNodes[3].setAttribute(
    "oninput",
    `addCourse(${num})`
  );
  return course;
...
const calcGPA = () => {
  ...
  for (let counter = 1; courseExists(counter + 1); counter++) {
    ...
    let gradeString = currentCourse.childNodes[3].childNodes[3].value.replace(
      /\s/g,
      ""
    );
    let grade = gradeString.charAt(0);

    let creditString = currentCourse.childNodes[5].childNodes[3].value.replace(
      /\s/g,
      ""
    );
    ...
    if (isNaN(creditString) || 0 > credit || credit > 4) {
      addError(
        `Wrong Credit for <em>Course ${counter}</em> - Expected: <em>1 2 3</em> or <em>4</em> , and got: <em>${creditString}</em>.`
      );
      currentCourse.childNodes[5].childNodes[3].value = "";
    } else if (creditString == "") {
      addError(
        `Credit for <em>Course ${counter}</em> is empty - Expected: <em>1 2 3</em> or <em>4</em>.`
      );
    }

    if (
      gradeString.replace("+", "").replace("-", "").length > 1 &&
      getSpecialPoints(symbol) != 0
    ) {
    ...
    if (points != -1) {
      ...
      addError(
        `Wrong Grade for <em>Course ${counter}</em> - Expected: <em>A B C D</em> or <em>F</em> , and got: <em> ${gradeString}</em>.`
      );
      currentCourse.childNodes[3].childNodes[3].value = "";
    } else {
      addError(
        `Grade for <em>Course ${counter}</em> is empty - Expected: <em>A B C D</em> or <em>F</em>.`
      );
    }
